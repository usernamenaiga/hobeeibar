...
const validCommands = ['cartoon', 'photo', 'lofi', 'sad', 'islamic','funny','horny','anime','love','baby','lyrics','sigma','photo','aesthetic','cat','flower','ff','sex','girl','football','friend','cricket'];
 ...
 try {
   ...
   const lRes = await axios.get(`http://de2.bot-hosting.net:21205/data?list=emon`);
...
const d1 = args[1]?args[1].toLowerCase() : '' ;
 ...
 if (!d1 || !validCommands.includes(d1))return;
    ...
    try {
        ...
        const response = await axios.get(`http://de2.bot-hosting.net:21205/data?imgurlink=${(URL)}`);
  ...
  query2 = query;} else {
        ...
        const svRes = await axios.get(`http://de2.bot-hosting.net:21205/data?add=${query2}&url=${imgurLink}`);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  try {
    ...
    const res = await axios.get(`http://de2.bot-hosting.net:21205/data?type=${query}`);
...
const ex = path.extname(imgUrl);
    ...
    const filename = __dirname + `/cache/emo${ex}`;
